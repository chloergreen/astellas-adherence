...
SELECT
id_pt,name, category,
COUNT(name) OVER (PARTITION BY id_pt) AS barrier_count
FROM base_query
)
Select id_pt, name, category, barrier_count,
CASE WHEN barrier_count = 1 THEN 1 
     WHEN barrier_count = 2 THEN 2
     WHEN barrier_count = 3 THEN 3
     WHEN barrier_count = 4 THEN 4
     WHEN barrier_count >= 5 THEN 5
ELSE null 
end barrier_bin
from barrier_count
