...
AND (dx1.code IN ('H35.3113', 'H35.3114', 'H35.3123', 'H35.3124', 'H35.3133', 'H35.3134')
    OR dx2.code IN ('H35.3113', 'H35.3114', 'H35.3123', 'H35.3124', 'H35.3133', 'H35.3134')
    OR dx3.code IN ('H35.3113', 'H35.3114', 'H35.3123', 'H35.3124', 'H35.3133', 'H35.3134'))
AND (pts_iz.tf_izervay_in_text IS NULL OR pts_iz.tf_izervay_in_text = FALSE) 
AND c.name IS NOT NULL  --ony patients with barriers?
),
--add deduplication, pts can have the same barriers multiple times
pt_barriers_dedup AS 
(
SELECT DISTINCT 
    id_pt,
    category
FROM pt_barriers
WHERE category != 'other'  -- no other because it is a broad category
),
...
FROM pt_barriers_dedup b1
INNER JOIN pt_barriers_dedup b2 
...
SELECT 
    ...
    ROUND(COUNT(*) * 100.0 / (
        ...
        FROM pt_barriers_dedup
    ), 2) as pct_of_ga_only_pts
FROM barrier_pairs
