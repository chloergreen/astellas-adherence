...
patient_barriers_dedup AS (
SELECT DISTINCT 
    id_pt,
    tf_izervay_in_text,
    category
FROM patient_barriers
...
SELECT 
    b1.id_pt,
    b1.tf_izervay_in_text,
    b1.category as barrier_1,
    b2.category as barrier_2
FROM patient_barriers_dedup b1
INNER JOIN patient_barriers_dedup b2 --self join? is this correct
    ON b1.id_pt = b2.id_pt 
...
SELECT 
    ...
    barrier_2,
    COUNT(*) as pair_count,
    COUNT(CASE WHEN tf_izervay_in_text = TRUE THEN 1 END) as izervay_pairs,
    COUNT(CASE WHEN tf_izervay_in_text = FALSE THEN 1 END) as ga_only_pairs,
    ROUND(COUNT(*) * 100.0 / (
        SELECT COUNT(DISTINCT id_pt) 
        FROM patient_barriers_dedup
    ), 2) as pct_of_all_patients,
    ROUND(COUNT(CASE WHEN tf_izervay_in_text = TRUE THEN 1 END) * 100.0 / (
        SELECT COUNT(DISTINCT id_pt) 
        FROM patient_barriers_dedup 
        WHERE tf_izervay_in_text = TRUE
    ), 2) as pct_of_izervay_patients,
    ROUND(COUNT(CASE WHEN tf_izervay_in_text = FALSE THEN 1 END) * 100.0 / (
        SELECT COUNT(DISTINCT id_pt) 
        FROM patient_barriers_dedup 
        WHERE tf_izervay_in_text = FALSE
    ), 2) as pct_of_ga_only_patients
FROM barrier_pairs
GROUP BY barrier_1, barrier_2
